...
Since the codebase uses modern ES6 modules, it must be hosted on a local web server (opening raw files locally is blocked by browser CORS security policies).

1.  Clone or download this repository.
2.  Open your terminal in the project directory.
3.  Start a local server using Python (included by default on macOS/Linux/Windows):
    ```bash
...
2.  In the repository settings, go to the **Pages** tab (on the left menu).
3.  Under **Build and deployment**, set the source to **Deploy from a branch**.
4.  Select your branch (e.g., `main` or `master`) and select the `/ (root)` folder.
5.  Click **Save**.
6.  Your site will be live at `https://<your-username>.github.io/<repository-name>/` in a few minutes! (Relative imports ensure all files resolve perfectly in subdirectories).
